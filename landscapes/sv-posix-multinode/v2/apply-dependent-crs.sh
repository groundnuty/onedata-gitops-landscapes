#!/usr/bin/env bash
# Day-2 companion script for sv-posix-multinode/v2 -- unchanged from
# v1's own apply-dependent-crs.sh other than NAMESPACE's default (see
# v1's own header comment for the full rationale: StorageBackend/User's
# onepanelEndpoint fields are the managed Oneprovider's/Onezone's
# onepanel POD IP, not knowable until that CR reaches Ready, so cannot
# be committed to git -- the same drive-first pattern
# demo/landscape-3p and demo/landscape-max already use).
#
# Run this ONCE, after `make deploy-landscape NAME=sv-posix-multinode
# VERSION=v2` and after both `onezone/zone` and `oneprovider/provider`
# report `Ready` in the sv-posix-multinode-v2 namespace.
#
...
NAMESPACE="${NAMESPACE:-sv-posix-multinode-v2}"
